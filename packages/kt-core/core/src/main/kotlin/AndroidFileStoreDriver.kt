...
class AndroidFileStoreDriver(
    ...
    private fun pathFor(digest: ContentDigest): File = File(directory, "${digest.type.value}_${digest.value_.hex()}")

    override suspend fun has(digest: ContentDigest): Boolean = withContext(Dispatchers.IO) { pathFor(digest).exists() }
